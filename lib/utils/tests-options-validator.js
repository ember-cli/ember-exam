...
/**
 * Validates the specified browsers
 * @param {*} browsers 
 * @param {*} loadBalance 
 */
function validateBrowser(loadBalance, browser) {
  if(!loadBalance) {
    throw new Error('You must use the \'load-balance\' option in order to use \'browser\'.');
  }

  if(isNaN(browser)) {
    throw new Error('You must specify a brower value numeric value.')
  }

  if (browser < 1) {
    throw new Error('You must specify a browser value greater than or equal to 1.')
  }
}
...
Object.defineProperty(TestsOptionsValidator.prototype, 'shouldLoadBalance', {
  get: function _shouldLoadBalance() {
    ...
    if (typeof browser !== 'undefined') {
      validateBrowser(loadBalance, browser)
    }
...
//validate browser option so that it's only being used with load-balance
