...
module.exports = class TestsOptionsValidator {
  ...
  validateCommands() {
    ...
    if (typeof this.options.writeExecutionFile !== 'undefined') {
      validatedOptions.set('writeExecutionFile', this.validateWriteExecutionFile());
    }
  ...
  validateWriteExecutionFile() {
    const shouldWriteExecutionFile = this.options.writeExecutionFile && this.options.loadBalance;

    if (this.options.writeExecutionFile && !this.options.loadBalance) {
      throw new SilentError(
    ...
    return shouldWriteExecutionFile;
  }
