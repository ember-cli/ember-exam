...
module.exports = class TestsOptionsValidator {
  ...
  validateCommands() {
    ...
    if (typeof this.options.executionOnly !== 'undefined') {
      validatedOptions.executionOnly = this.validateExecutionOnly();
    }
   ...
   *  @return {boolean}
   */
  validateExecutionOnly() {
    const shouldExecuteOnly = this.options.executionOnly && this.options.loadBalance;

    if (this.options.executionOnly && !this.options.loadBalance) {
      throw new SilentError(
        'EmberExam: You must run test suite with the `load-balance` option in order to use the `execution-only` option.'
      );
    }
