...
TestsProcessor.prototype.split = function() {
  ...
  this.testGroups = ArrayUtils.split(this.testModules, options.split);

  // If a `split-file` has been specified, move it to the front so that it
  // gets ran during testing
  var splitFile = options.splitFile;
  if (splitFile) {
    ArrayUtils.moveElement(this.testGroups, splitFile - 1, 0);
  }
};
