...
function isTest(name) {
  return name.indexOf('-test', name.length - 5) > -1;
}

function moduleIsTest(module) {
  var moduleName = module.expression.arguments[0].value;
  return moduleName && isTest(moduleName);
}

function moduleIsNotTest(module) {
  return !moduleIsTest(module);
}
...
TestsProcessor.prototype._divideAST = function() {
  ...
  // Extract non-test modules. We assume that any module that is not a test
  // should be included in all test bundles. This would apply to helpers,
  // fixtures, and other test scaffolding.
  this.nonTestModules = body.filter(moduleIsNotTest);
  this.testModules = body.filter(moduleIsTest);
};
...
TestsProcessor.prototype.write = function() {
  if (this.testGroups) {
    ...
    // Overwrite the tests.js file with our custom loader and the non-test modules
    this.ast.program.body = this.nonTestModules;
    var nonTestModules = recast.print(this.ast).code
    var testLoaderFile = fs.readFileSync(path.resolve(__dirname, '../tests.js'), { encoding: 'utf-8' });
    fs.writeFileSync(this.testsFilePath, nonTestModules + '\n' + testLoaderFile);
  } else {
    var content = this.nonTestModules.concat(this.testModules);
