...
class ExecutionStateManager {
  ...
  addToModuleMap(moduleName, browserId) {
    let browserIdModuleMap = this._browserToModuleMap.get(moduleName);
    if (Array.isArray(browserIdModuleMap)) {
      browserIdModuleMap.push(moduleName);
    } else {
      browserIdModuleMap = [moduleName];
    }
    this._browserToModuleMap.set(browserId, browserIdModuleMap);
  }
