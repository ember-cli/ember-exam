...
class ExecutionStateManager {
  ...
  getBrowserModuleQueue(browserId) {
    if (this._browserModuleQueue) {
      return this._browserModuleQueue.get(browserId);
    }
    return null;
  }
  ...
  setBrowserModuleQueue(moduleQueue, browserId) {
    ...
    this._browserModuleQueue.set(browserId, moduleQueue.slice());
  }
  ...
  getNextModuleBrowserModuleQueue(browserId) {
    if (this._browserModuleQueue && this._browserModuleQueue.get(browserId)) {
      return this._browserModuleQueue.get(browserId).shift();
    }
    return null;
  }
   ...
   * @param {number} browserId
   */
  addModuleNameToModuleMap(moduleName, browserId) {
