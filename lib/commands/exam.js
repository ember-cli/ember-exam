...
module.exports = TestCommand.extend({
  ...
  _addLoadBalancingBrowserSocketEvents(config) {
    ...
    const getBrowserId = function(testPage) {
      ...
      if (Array.isArray(browserId = /browser=\s*([0-9]*)/.exec(testPage) !== null)) {
        return browesrId[1];
      } else {
    ...
    events['testem:get-next-test-module'] = function() {
      const browserId = getBrowserId(this.launcher.settings.test_page);
    ...
    if (commands.loadBalance) {
      ...
      const browserFailureHandler = function() {
        ...
        browserExitHandler(this.config.appMode);
      };

      const browserExitHandler = function (appMode) {
        const browserCount = Object.keys(config.testPage).length;

        if (testemEvents.completedBrowsersHandler(browserCount, commands.loadBalance, testExecutionFileName, appMode)) {
          ui.writeLine(`\nThis execution was recorded at ${testExecutionFileName}`);
        }
      };

      events['start-tests'] = function() {
        this.process.on('processExit', browserFailureHandler);
        this.process.on('processError', browserFailureHandler);
      }
