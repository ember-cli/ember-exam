...
module.exports = TestCommand.extend({
  ...
  _addLoadBalancingBrowserSocketEvents(config) {
    ...
    const getBrowserId = function(testPage) {
      // if noExecutionFile is true, the browserId should not matter, returning 0
      if (commands.noExecutionFile) return 0;
    ...
    if (commands.get('loadBalance')) {
      ...
      const browserExitHandler = () => {
        ...
        if (testemEvents.completedBrowsersHandler(browserCount, commands.get('loadBalance'), testExecutionFileName, commands.get('noExecutionFile'))) {
      ...
      events['tests-start'] = function() {
        ...
        if (!registerProcessOn) {
          if (this.process && this.launcher.settings.test_page) {
