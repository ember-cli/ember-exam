...
module.exports = TestCommand.extend({
  ...
  availableOptions: [
    ...
    { name: 'replay-browser',       type: [Array, Number, String], aliases: ['rb'], description: 'The number of the browser(s) to run from a specified file path' },
    { name: 'write-execution-file', type: Boolean, default: false, aliases: ['wef'],description: 'Allows writing a test-execution json file after running your test suite'}
  ...
  _addLoadBalancingBrowserSocketEvents(config) {
    ...
    const browserExitHandler = function () {
      ...
      testemEvents.completedBrowsersHandler(browserCount, ui, commands.get('loadBalance'), testExecutionFileName, commands.get('writeExecutionFile'));
    };
    ...
    events['testem:next-module-request'] = function (browserId) {
      testemEvents.nextModuleResponse(browserId, this.socket, commands.get('loadBalance'), commands.get('writeExecutionFile'));
    };
