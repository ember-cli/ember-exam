...
module.exports = TestCommand.extend({
  ...
  run(commandOptions) {
    ...
    commandOptions.replayBrowser = this._convertToArray(commandOptions.replayBrowser);
  ...
  getExecutionMappingInstance(options) {
    ...
    try {
      ...
      const errorMessage = `Failed to read ember-exam replay file: ${executionFilePath} ` + err.message;
    ...
    if (Array.isArray(optionValue)) {
      optionValue.forEach((element) => {
        if (typeof element === 'string') {
          Array.prototype.push.apply(optionArray, element.split(','));
        } else {
  ...
  /**
   * Appends parameter to baseUrl.
   *
   * @param {*} baseUrl
   */
  _appendParamToBaseUrl(commandOptions, baseUrl) {
    if (this.validator.shouldParallelize || this.validator.shouldSplit) {
    ...
    if (this.validator.shouldLoadBalance || this.validator.shouldReplayExecution) {
      const partitions = commandOptions.partition;
  ...
  _getCustomBaseUrl(config, commandOptions) {
    ...
    if (Array.isArray(baseUrl)) {
      return baseUrl.map((currentUrl) => {
        return this._appendParamToBaseUrl(commandOptions, currentUrl);
      })
    } else {
      return this._appendParamToBaseUrl(commandOptions, baseUrl);
    }
  },
