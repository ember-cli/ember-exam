...
module.exports = TestCommand.extend({
  ...
  _generateCustomConfigs(commandOptions) {
    ...
    if (this.validator.shouldParallelize) {
      ...
      const count = commandOptions.split;
   ...
   */
  _generateTestPages(baseUrl, partitions, count) {
    const splitUrl = addToUrl(baseUrl, '_split', count);
    const testPages = [];

    for (let i = 0; i < partitions.length; i++) {
      testPages.push(addToUrl(splitUrl, '_partition', partitions[i]));
    }
