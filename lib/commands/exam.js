...
module.exports = TestCommand.extend({
  ...
  _addLoadBalancingBrowserSocketEvents(config) {
    ...
    const getBrowserId = function(testPage) {
      // when running `--no-launch` a test execution file is not created.
      if (testPage === undefined) {
        commands.noExecutionFile = true;
      } else {
    ...
    events['testem:set-modules-queue'] = function(modules) {
      ...
      testemEvents.setModuleQueue(browserId, modules, commands.loadBalance, commands.replayExecution);
    }

    events['testem:next-module-request'] = function() {
      const browserId = getBrowserId(this.launcher.settings.test_page);
      testemEvents.nextModuleResponse(browserId, this.socket, commands.loadBalance, commands.noExecutionFile);
    }
    ...
    if (commands.loadBalance) {
      ...
      const browserExitHandler = () => {
        ...
        if (testemEvents.completedBrowsersHandler(browserCount, commands.loadBalance, testExecutionFileName, commands.noExecutionFile)) {
