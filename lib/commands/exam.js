...
module.exports = TestCommand.extend({
  ...
  availableOptions: [
    ...
    { name: 'no-execution-file',type: Boolean, default: false, aliases: ['nef'],description: 'Disables writing a test-execution json file after running your test suite'}
  ...
  init() {
    ...
    this.emberCliVersion = (this.project.pkg.devDependencies['ember-cli'] || this.project.pkg.dependencies['ember-cli'])
  },
  ...
  _validateOptions(commandOptions) {
    ...
    const validator = new Validator(commandOptions, this._getTestFramework(), this.emberCliVersion);
  ...
  _addLoadBalancingBrowserSocketEvents(config) {
    ...
    const browserExitHandler = function () {
      ...
      testemEvents.completedBrowsersHandler(browserCount, ui, commands.get('loadBalance'), testExecutionFileName, commands.get('noExecutionFile'));
    };
    const browserFailureHandler = function () {
      if (this.finished){
        return;
      } else if (!commands.get('noExecutionFile')) {
    ...
    events['tests-start'] = function () {
      if (!init) {
        // if this.process or this.launcher.setting.test_page is undefined, it means `no-launch` was passed
        // when running with `--no-launch`, a test execution file is not created
        // because Testem does not create browser instance, which is needed to correctly
        // book keep the test execution
        // TODO: use --no-launch here instead of checking this.process & this.launcher.setting.test_page
        const browserLaunched = (this.process && !commands.get('noLaunch'));
        if (!browserLaunched || commands.get('replayExecution')) {
          commands.set('noExecutionFile', true);
        } else {
    ...
    events['testem:set-modules-queue'] = function (modules, browserId) {
      testemEvents.setModuleQueue(browserId, modules, commands.get('loadBalance'), commands.get('replayExecution'));
    };
    events['testem:next-module-request'] = function (browserId) {
      testemEvents.nextModuleResponse(browserId, this.socket, commands.get('loadBalance'), commands.get('noExecutionFile'));
    };
    events['test-result'] = function (result) {
      if (result.failed && !commands.get('noExecutionFile')) {
        const browserId = getBrowserId(this.launcher.settings.test_page);
        testemEvents.recoredFailedBrowserId(browserId);
      }
    };
