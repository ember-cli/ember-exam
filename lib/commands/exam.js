...
module.exports = TestCommand.extend({
  ...
  availableOptions: [
    ...
    { name: 'load-balance', type: Boolean, default: false, description: 'Load your test balanced' },
    { name: 'browser',      type: Number,                  description: 'The number of the browser(s) to serve.' },
  ...
  _generateCustomConfigs: function(commandOptions) {
    ...
    const parallel = this.validator.shouldParallelize;
    const loadBalance = this.validator.shouldLoadBalance;
    const serveMultipleBrowsers = parallel || loadBalance;

    if (!serveMultipleBrowsers) {
      return config;
    }

    let id = commandOptions.partition;
    let appendingParam = '_partition';
    // if 
    if (parallel) {
      let partitions = commandOptions.partition;
    ...
    if (loadBalance) {
      let browserCount = commandOptions.browser;
      appendingParam = '_browser';

      if (!browserCount) {
        browserCount = 1;
      }
      id = Array.from({length: browserCount}, (v,k) => k+1); // if browser=3 then id = [1,2, 3]
    }

    if (Array.isArray(customBaseUrl)) {
      const command = this;
      config.testPage = customBaseUrl.reduce(function(testPages, customBaseUrl) {
        return testPages.concat(command._generateTestPages(customBaseUrl, appendingParam, id));
      }, []);
    } else {
      config.testPage = this._generateTestPages(customBaseUrl, appendingParam, id);
    }
   ...
   * @param {String} appendingParam // find a better name that indicates meaning of it.
   * @param {Array<Number} browserId
   * @return {Array<String>} testPages
   */
  _generateTestPages: function(customBaseUrl, appendingParam, id) {
    const testPages = [];
    for (let i = 0; i < id.length; i++) {
      const url = addToUrl(customBaseUrl, appendingParam, id[i]);
      testPages.push(url);
    }
