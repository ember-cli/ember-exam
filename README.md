...
`load-balance` allows you to load balance test files against. It will order the test files by test type and load balance the ordered test files between the browsers dynamically rather than statically. `parallel` must be used along with `load-balance` to specify a number of browser(s) and `load-balance` can not be used without `parallel` being specified a number of browser(s).

This option was added to version 1.1 to address execution performance when running against a large test suite.

A way of communication between server and browser to send and receive a test module to load is promise based. Its timeout value is set to be 2 seconds and the timeout can be customized by user by adding `asyncTimeout=[timeout]` in passing url or adding to a testem config.
...
`load-balance` is currently supported in CI mode and due to the reason `no-launch` can not be passed with `load-balance`.

**Note 1** _You must be using ember-cli version 3.2.0 or greater for load-balnce and test failure reproduction features to work properly._
**Note 2** _You must be using ember-qunit version 4.1.1 or greater for this feature to work properly._
**Note 3** _You must be using qunit version 2.8.0 or greater for this feature to work properly._
**Note 4** _This feature is not currently supported by Mocha._

##### Test Failure Reproduction

Due to the dynamic nature of the `load-balance` option, test file execution order could vary between runs. To reproduce a past test execution the execution must be recorded and passing `--write-execution-file`, in short `--wef` allows generating a JSON file which enables to rerun the past test execution. The option is only allowed when `load-balance` is passed.

```bash
# The command will load test balanced with serve <num> of browser(s). After the test suite execution It will generate a test-execution json file.
$ ember exam --parallel=<num> --load-balance --wef
$ ember exam --parallel=<num> --load-balance --write-execution-file
```

The file is stored in the root directory and the naming structure is `test-execution-YYYY-MM-DD--HH-MM-SS.json`.
To replay the test execution for one particular browser, do the following:

```bash
# The command will read test execution file passed for `replay-execution` and execute a browser Id(s) specified for `rplay-browser`
$ ember exam --replay-execution=[string] --replay-browser=[num]
```

`replay-execution` allows you to specify a path to the json file to run execution against and `replay-browser` is to specify browser IDs to execute.

```bash
# The command will read test-execution-000000.json and load the list of module mapped to browserId 1
...
`replay-browser` can be an array of broweser IDs. For instance `--replay-broweser=1,2` will bring two browsers and execute a list of modules which ran by previous browser #1 and #2.
...
When `replay-browser` value is not specified its value is set to be a value of `failedBrowser` from the specified `replay-execution` file.

```bash
# The command will read test-execution-000000.json and load the list of module mapped to browserIds from failedBrowser in the json file.
$ ember exam --replay-execution=test-execution-000000.json
```

When `replay-browser` value is not specified and there is no value for `failedBrowser` in the json file it will execute all list of modules.
